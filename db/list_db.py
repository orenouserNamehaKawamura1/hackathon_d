...
def ac_lists(page_num,per_page):
    sql = 'SELECT id,mail FROM users WHERE delete_flag = false ORDER BY id asc limit %s OFFSET %s'
...
def img_list(page_num,per_page):
    sql = 'SELECT img_path FROM images WHERE delete_flag = false ORDER BY id asc limit %s OFFSET %s'
